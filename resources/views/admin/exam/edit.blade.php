        ...
        <div class="row justify-content-center">
            <div class="col-md-6">
              ...
              {{-- <form action="" method="POST" enctype="multipart/form-data">
                ...
                <form action="{{ route('dashboard.exam.update', $editdata->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card">
                        <div class="card_header d-flex flex-wrap gap-3 justify-content-between p-4">
                            <h2>Studented Create</h2>
                        ...
                        <div class="card-body">
                            ...
                            <div class="from-group col-md-6">
                                ...
                                <input value="{{ $editdata->exam_name }}" type="text" name="exam_name" class="form-control" />
                                @error('name')
                            ...
                            <div class="form-group mt-6">
                                ...
                                @foreach ($data as $item)
                                    <label for="exam{{ $item->id }}">
                                        <input   {{ $editdata->examstudentd->firstwhere('id', '=', $item->id) ? 'checked' : '' }}  type="checkbox" value="{{ $item->id }}" id="exam{{ $item->id }}"
                                            name="student_name[]">
                                        {{ $item->name }}
                                    </label>
                                @endforeach
