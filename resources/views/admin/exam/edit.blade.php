...
<div class="container">
    ...
    <div class="row justify-content-center">
        <div class="col-md-6">
            <form action="{{route('dashboard.studented.update',$editdata->id)}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card">
                    <div class="card_header d-flex flex-wrap gap-3 justify-content-between p-4">
                        <h2>student Edit</h2>
                         <a href="{{route('dashboard.studented.index')}}" class="btn btn-outline-info"> <i class="fa fa-arrow-left">Back</i></a>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="from-group col-md-6">
                                ...
                                <input value="{{$editdata->email}}" type="text" name="email" class="form-control" />
                                @error('role')
                                <div class="text-danger">{{$message}}</div>
                                @enderror
                          </div>
                          <div class="from-group col-md-6">
                            ...
                            <select class="form-control" name="department">
                                @foreach ($all_data as $item)
                                    <option {{ $editdata->department == $item->id ? 'selected':''}} value="{{ $item->id }}">{{ $item->department }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="from-group col-md-6">
                            <label for=""> Registation</label>
                            <input value="{{$editdata->registation}}" type="text" name="registation" class="form-control" />
                            @error('role')
                            <div class="text-danger">{{$message}}</div>
                            @enderror
                      </div>
